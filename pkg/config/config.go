...
func UpdateConfig(ctx context.Context, bgpServer *server.BgpServer, c, newConfig *oc.BgpConfigSet) (*oc.BgpConfigSet, error) {
	...
	// Netlink config is refreshed here, and Vrfs is carried too - not just
	// Netlink. UpdateConfig never refreshed Vrfs, so a SIGHUP changing a VRF's
	// netlink-import or netlink-export block never reached buildVrfMappings and
