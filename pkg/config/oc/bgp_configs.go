...
type NetlinkExportRule struct {
	...
	// Match paths carrying any of these communities. Empty matches
	// every path.
	...
	// original -> gobgp:table-id
	// Linux routing table to program. 0 means the main table.
	TableId int32 `mapstructure:"table-id" json:"table-id,omitempty"`
	// original -> gobgp:metric
	Metric uint32 `mapstructure:"metric" json:"metric,omitempty"`
	// original -> gobgp:skip-nexthop-validation
	// gobgp:skip-nexthop-validation's original type is boolean.
	// Skip the reachability check before programming a route. Stated as
	// skip- rather than validate- so that an absent value and an explicit
	// false mean the same safe thing: validate.
	SkipNexthopValidation bool `mapstructure:"skip-nexthop-validation" json:"skip-nexthop-validation,omitempty"`
}
...
func (lhs *NetlinkExportRule) Equal(rhs *NetlinkExportRule) bool {
	...
	if lhs.SkipNexthopValidation != rhs.SkipNexthopValidation {
		return false
	}
	return true
}

// struct for container gobgp:export.
type NetlinkExport struct {
	// original -> gobgp:enabled
	// gobgp:enabled's original type is boolean.
	Enabled bool `mapstructure:"enabled" json:"enabled,omitempty"`
	// original -> gobgp:dampening-interval
	// Milliseconds to coalesce repeated updates for a prefix before
	// programming it.
	DampeningInterval uint32 `mapstructure:"dampening-interval" json:"dampening-interval,omitempty"`
	// original -> gobgp:route-protocol
	// RTPROT_* value written on exported routes, and the value the startup
	// sweep matches on. 186 is RTPROT_BGP and is shared with other BGP
	// daemons; give this daemon a dedicated value or a dedicated table if
	// another one writes to the same table.
	RouteProtocol int32 `mapstructure:"route-protocol" json:"route-protocol,omitempty"`
	// original -> gobgp:rules
	// Export rules. A path is exported if it matches any rule.
...
// struct for container gobgp:import.
type NetlinkImport struct {
	// original -> gobgp:enabled
	// gobgp:enabled's original type is boolean.
	// Configure enabling netlink route exchange.
	Enabled bool `mapstructure:"enabled" json:"enabled,omitempty"`
	// original -> gobgp:vrf
	// Name of the GoBGP VRF this applies to. Empty means the global RIB.
	Vrf string `mapstructure:"vrf" json:"vrf,omitempty"`
	// original -> gobgp:interface
	// Interfaces to read connected routes from. Shell-style globs are
	// expanded against the interfaces present on the system.
...
type VrfNetlinkExport struct {
	...
	// gobgp:enabled's original type is boolean.
	Enabled bool `mapstructure:"enabled" json:"enabled,omitempty"`
	// original -> gobgp:linux-vrf
	// Linux VRF device to program routes into.
	LinuxVrf string `mapstructure:"linux-vrf" json:"linux-vrf,omitempty"`
	// original -> gobgp:linux-table-id
	// Linux routing table id. Resolved from the VRF device when zero.
	LinuxTableId int32 `mapstructure:"linux-table-id" json:"linux-table-id,omitempty"`
	// original -> gobgp:metric
	Metric uint32 `mapstructure:"metric" json:"metric,omitempty"`
	// original -> gobgp:skip-nexthop-validation
	// gobgp:skip-nexthop-validation's original type is boolean.
...
func (lhs *VrfNetlinkImport) Equal(rhs *VrfNetlinkImport) bool {
	if lhs == nil || rhs == nil {
		return false
	}
	if lhs.Enabled != rhs.Enabled {
...
type Vrf struct {
	...
	// original -> gobgp:vrf-netlink-import
	// Netlink import configuration for this VRF.
	NetlinkImport VrfNetlinkImport `mapstructure:"netlink-import" json:"netlink-import,omitempty"`
	// original -> gobgp:vrf-netlink-export
	// Netlink export configuration for this VRF.
